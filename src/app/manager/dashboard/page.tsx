...
interface ManagerDashboardResponse {
  managerDashboard?: {
    ...
    newDoctorsThisMonth?: number | null;
    pendingDoctors?: number | null;
    salesAmountPastMonth?: number | null;
    salesAmountThisMonth?: number | null;
...
function ManagerDashboardContent() {
  ...
  } = useQuery<ManagerDashboardResponse>(MANAGER_DASHBOARD, {
  ...
  if (dashboardError) {
    console.error("Failed to load manager dashboard:", dashboardError);
  }
  if (ordersGraphError) {
  ...
  const d = dashboardData?.managerDashboard;
  const stats = [
    {
      label: "Total Doctors",
      value: dashboardLoading ? "..." : formatNumber(d?.totalDoctors),
      icon: (
        <DashDoctor
    ...
    },
    {
      ...
      value: dashboardLoading ? "..." : formatNumber(d?.activeDoctors),
      icon: (
        <DashDoctor
          height={isMobile ? "16" : "32"}
          width={isMobile ? "16" : "32"}
    ...
    },
    {
      ...
      value: dashboardLoading ? "..." : formatNumber(d?.pendingDoctors),
      icon: (
        <DashDoctor
          height={isMobile ? "16" : "32"}
          width={isMobile ? "16" : "32"}
    ...
    },
    {
      ...
      value: dashboardLoading ? "..." : formatNumber(d?.newDoctorsThisMonth),
      icon: (
        <DashDoctor
          height={isMobile ? "16" : "32"}
          width={isMobile ? "16" : "32"}
    ...
    },
    {
      ...
      value: dashboardLoading ? "..." : formatNumber(d?.totalProductsSold),
      icon: (
        <PackageIcon
          fill="#fff"
          height={isMobile ? 16 : 32}
    ...
    {
      label: "Sales Today",
      value: dashboardLoading ? "..." : formatCurrency(d?.salesAmountToday),
      icon: (
        <OrdersIcon
          fill="#fff"
          height={isMobile ? "16" : "32"}
    ...
    },
    {
      ...
      value: dashboardLoading ? "..." : formatCurrency(d?.salesAmountThisMonth),
      icon: (
        <InventoryIcon
          fill="#fff"
          height={isMobile ? "16" : "32"}
    ...
    },
    {
      ...
      value: dashboardLoading ? "..." : formatCurrency(d?.salesAmountPastMonth),
      icon: (
        <CustomerIcon
          fill="#fff"
          height={isMobile ? "16" : "32"}
    ...
    <div className="lg:max-w-7xl md:max-w-6xl w-full flex flex-col gap-4 md:gap-6 pt-2 mx-auto">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4 md:gap-6">
        <div className="bg-white rounded-xl shadow-table p-4 md:p-6">
          ...
          ) : d?.topPerformingDoctors && d.topPerformingDoctors.length > 0 ? (
            <div className="flex flex-col gap-3">
              {d.topPerformingDoctors.map((doctor, index) => (
                <div
                  key={doctor.doctorId ?? doctor.doctorEmail}
        ...
        <div className="bg-white rounded-xl shadow-table p-4 md:p-6">
          ...
          ) : d?.newlyOnboardedDoctors && d.newlyOnboardedDoctors.length > 0 ? (
            <div className="flex flex-col gap-3">
              {d.newlyOnboardedDoctors.map((doctor, index) => (
                <div
                  key={doctor.doctorId}
                  className="flex items-center justify-between gap-3 p-3 rounded-lg bg-gray-50 transition-colors"
                >
