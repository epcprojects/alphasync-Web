...
/** Manager orders: same as admin but customer column and customer detail hidden. */
function ManagerOrdersContent() {
  ...
  /** No customer column: Order ID | Doctor | Payment Date | Status | Shipment | Items | Total | Net Cost | Profit | Actions */
  const managerOrdersGrid =
    "md:grid-cols-[1.5fr_1.5fr_1.5fr_1fr_1fr_1fr_1.5fr_1fr_1fr_1fr]";
  /** Pending: no Payment Date or Shipment — Order ID | Doctor | Status | Items | Total | Net Cost | Profit | Actions */
  const managerOrdersGridPending =
    "md:grid-cols-[1.5fr_1.5fr_1fr_1fr_1.5fr_1fr_1fr_1fr]";
  ...
  const handleExportOrders = async () => {
    try {
      ...
      if (result?.success && result?.csvString) {
        ...
        const fileName = `manager-orders-${myClinic ? "clinic" : "customer"}-${new Date().toISOString().slice(0, 10)}.csv`;
            ...
            {orderTabs.map((tab) => (
              <TabPanel key={tab.label}>
                <div className="space-y-1 p-0 md:p-4 pt-0">
                  <div
                    className={`hidden md:grid ${tab.pendingPayment ? managerOrdersGridPending : managerOrdersGrid} text-black font-medium text-sm gap-4 px-2 py-2.5 bg-white rounded-xl shadow-table`}
                  >
                    <div>
                        ...
                        return (
                          <OrderListView
                            ...
                            order={{
                              ...
                              customer: doctorName ?? "—",
                              doctorName,
                              imageUrl: order.doctor?.imageUrl ?? null,
                              customerEmail: null,
                              date: (() => {
